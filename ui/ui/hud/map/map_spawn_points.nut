...
let { selectedSpawn } = require("%ui/gameModeState.nut")
...
function mkSpawnsPoly(spawnInfo, zoneInfo, alpha, radius, map_size, clampParam, transform) {
  let N = 4
  ...
  let sf = Watched(0)
  let isHovered = Computed(@() (sf.get() & S_HOVER) && selectedSpawn.get()?.spawnGroupId == spawnInfo.spawnGroupId)
  ...
  return function() {
    ...
    function onMouseMove(mouseEvent) {
      ...
      if ((sf.get() & S_HOVER) && pointInPolygon(relativePoint, relativePoints)){
        selectedSpawn.set(spawnInfo)
        setTooltip(hoverTextPoly(spawnInfo.locId))
      } else if (selectedSpawn.get()?.spawnGroupId == spawnInfo.spawnGroupId) {
        selectedSpawn.set(null)
        setTooltip(null)
      }
    }
    ...
    return {
      watch = [ currentMapVisibleRadius, isHovered ]
      data = {
      ...
      size = polygonSize
      transform
      color = mul_color(TextNormal, isHovered.get() ? 0.8 : 0.6)
      fillColor = mul_color(TextNormal, isHovered.get() ? 0.2 : 0.1)
    ...
    children.extend(
      spawns.map(
        function(s) {
          ...
          return mkSpawnsPoly(s, zoneInfo, alpha, radius, map_size, raidDesc?.drawSpawnsClampParam, transform)
        }
      ).values())
  }
