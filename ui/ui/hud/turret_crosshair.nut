...
from "weapons.behaviors" import TurretCrosshair

let {controlledVehicleEid} = require("%ui/hud/state/vehicle_state.nut")
let {mainTurretEid, vehicleTurrets} = require("%ui/hud/state/vehicle_turret_state.nut")
...
function crosshair() {
  if (!vehicleTurrets.get().turrets.reduce(@(acc, v) acc || v.isControlled, false))
    return { watch = vehicleTurrets }
  return {
    watch = [controlledVehicleEid, mainTurretEid, vehicleTurrets]
    size = crossHairSize
    hplace = ALIGN_CENTER
    vplace = ALIGN_CENTER
    halign = ALIGN_CENTER
    valign = ALIGN_CENTER
    behavior = TurretCrosshair
    transform = {}

    eid = controlledVehicleEid.get()
    turretEid = mainTurretEid.get()
