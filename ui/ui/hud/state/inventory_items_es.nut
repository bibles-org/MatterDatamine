...
let { item_comps, getItemInfo, get_item_info, get_nearby_item_info } = require("%ui/hud/state/item_info.nut")
let { is_quest_item_for_other_player,  is_item_hidden_for_player } = require("das.inventory")
let { watchedHeroEid } = require("%ui/hud/state/watched_hero.nut")
let { EventCapacityExceeded, NotifyItemRecognitionStarted } = require("dasevents")
let { sound_play } = require("%dngscripts/sound_system.nut")
let { HERO_ITEM_CONTAINER, STASH, BACKPACK0, SAFEPACK} = require("%ui/hud/menus/components/inventoryItemTypes.nut")
...
function updateInventoryContainerCommon(inventory_comp_hash, inventory_items, items, get_item_info_cb) {
  let resInventory = []
  ...
  foreach (item_desc in items){
    ...
    if (is_quest_item_for_other_player(item.eid, localPlayerEid) || is_item_hidden_for_player(item.eid, localPlayerEid))
      continue
    item.inventoryEid <- item.owner
    item.inventoryCompHash <- inventory_comp_hash
    resInventory.append(item)
  }
  let prevItems = inventory_items.value
  ...
  if (changed){
    inventory_items(resInventory)
  }
}

function updateInventory(
  inventory_comp_hash, inventory_items, items) {
  updateInventoryContainerCommon(
    inventory_comp_hash, inventory_items, items, @(item_eid) item_eid)
}

function updateEidInventoryContainer(
  inventory_comp_hash, inventory_items, items) {
  updateInventoryContainerCommon(
    inventory_comp_hash, inventory_items, items, @(item_eid) get_item_info(item_eid))
}

let track_comps = [
  ["item__useTime", ecs.TYPE_FLOAT, null],
  ["item__volume", ecs.TYPE_FLOAT, null],
...
ecs.register_es("items_ammo_count_anim_start_es",
  {
    [["onChange", "onInit"]] = function(_evt, eid, comp) {
      let triggerName = comp.boxedItem != null && comp.boxed_item__template != null ?
        $"inventory_item_count_anim_{ecs.calc_hash(comp.boxed_item__template)}_ow_{comp.item__containerOwnerEid}" : 
...
inventoryItemsByEid.subscribe(@(v) updateInventory(ecs.calc_hash(HERO_ITEM_CONTAINER.containerName), inventoryItems, v))
backpackItemsByEid.subscribe(@(v) updateInventory(ecs.calc_hash(BACKPACK0.containerName), backpackItems, v))
safepackItemsByEid.subscribe(@(v) updateInventory(ecs.calc_hash(SAFEPACK.containerName), safepackItems, v))
stashItemsByEid.subscribe(@(v) updateInventory(ecs.calc_hash(STASH.containerName), stashItems, v))
...
function mkTrackItemsAround() {
  local _itemsAround = []
  let _get_items_around = throttle(@() itemsAround(_itemsAround.map(get_nearby_item_info).filter(@(v) v!=null && v.isPickable)), rebuildItemsAroundTime, {leading=false, trailing=true})
