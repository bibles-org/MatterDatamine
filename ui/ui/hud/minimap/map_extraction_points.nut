...
let { MapIconEnable, MapIconDisable, MapIconInner, MapIconHover, MapIconInactive } = require("%ui/components/colors.nut")
...
let extractionPoint = function(data, transform){
  ...
  function timerExtractionDisabledAtBlock(sf) {
    let { disableTime = 0 } = data
    local timer = Watched(0)
    timer = mkCountdownTimerPerSec(Watched(disableTime))
    let isExtractionEnabled = Computed(@() timer.get() > 0)
    return @() {
      watch = isOnPlayerBase
      flow = FLOW_VERTICAL
