...
let removeModalPopup = @(uid) removeModalWindow(uid)
...
function addModalPopup(rectOrPos, popup) {
  popup = POPUP_PARAMS.__merge(popup)
  popup.uid = popup?.uid ?? $"modal_popup_{lastPopupIdx++}"
  popup.hotkeys = popup.hotkeys ?? [["^{0} | Esc".subst(JB.B), { action = @() removeModalPopup(popup.uid), description = loc("Cancel") }]]
