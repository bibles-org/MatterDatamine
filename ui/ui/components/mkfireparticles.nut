...
let { gradRadial } = require("%ui/helpers/gradients.nut")
...
function fillNewForce(state, time, speedY) {
  state.accChangeTime <- time + (hdpx(50) / fabs(speedY) * rand.rfloat(100, 1000)).tointeger()
  let normSpeed = speedY * speedY / hdpx(100)
  let randomAcc = rand.rfloat(-normSpeed, normSpeed)
...
function updateParticles(state, halfSize, particleSize, speedY, scaleBySpeed, opacity) {
  let { isActive = false } = state
  let time = get_time_msec()

  if (!isActive) {
    initSparkState(state, halfSize, time, speedY, true)
    return { opacity = 0.0 }
  }
  else
