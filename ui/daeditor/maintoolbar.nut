...
let {showEntitySelect, showLoadedScenes, propPanelVisible, propPanelClosed, showHelp, de4editMode,
...
function mainToolbar() {
  ...
  function toggleLoadedScenes() {
    if (getEditMode() == DE4_MODE_CREATE_ENTITY || getEditMode() == DE4_MODE_POINT_ACTION)
      setEditMode(DE4_MODE_SELECT)
    showLoadedScenes.update(!showLoadedScenes.value);
  }
  ...
  return {
    ...
    children = [
      ...
      separator
      toolbarButton(svg("scenes"), toggleLoadedScenes, "Loaded scenes (I)", showLoadedScenes.value)
      separator
