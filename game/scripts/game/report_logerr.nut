...
let { mkCmdEnableDedicatedLogger, CmdEnableDedicatedLogger, mkEventDebugChatMessageAllClients } = require("%sqGlob/sqevents.nut")
...
function sendErrorToClient(_tag, logstring, _timestamp) {
  ...
  ecs.g_entity_mgr.broadcastEvent(mkEventDebugChatMessageAllClients({ text = logstring }))
}
